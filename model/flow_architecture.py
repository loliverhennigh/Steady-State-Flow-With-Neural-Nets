...
def calc_velocity(sflow):
 
  velocity_x =  sflow[:,:,:,2] - sflow[:,:,:,4] + sflow[:,:,:,5] + sflow[:,:,:,6] - sflow[:,:,:,7] - sflow[:,:,:,8]
  velocity_y =  sflow[:,:,:,1] - sflow[:,:,:,3] + sflow[:,:,:,5] - sflow[:,:,:,6] - sflow[:,:,:,7] + sflow[:,:,:,8]
...
def boundary_edge(boundary):
  ...
  # compute boundary shifts for all 8 ejacent cells
  boundary_shift_1 = boundary[:,1:  , :  ,:]
  boundary_shift_2 = boundary[:, :  ,1:  ,:]
  boundary_shift_3 = boundary[:, :-1, :  ,:]
  boundary_shift_4 = boundary[:, :  , :-1,:]
  boundary_shift_5 = boundary[:,1:  ,1:  ,:]
  boundary_shift_6 = boundary[:, :-1,1:  ,:]
  boundary_shift_7 = boundary[:, :-1, :-1,:]
  boundary_shift_8 = boundary[:,1:  , :-1,:]

  padding_1 = [[0,0], [1,2], [1,1], [0,0]]
  boundary_shift_1_kill  = tf.pad(tf.maximum(boundary_shift_1 - boundary_shift_3, 0.0), padding_1)
  padding_2 = [[0,0], [1,1], [1,2], [0,0]]
  boundary_shift_2_kill  = tf.pad(tf.maximum(boundary_shift_2 - boundary_shift_4, 0.0), padding_2)
  padding_3 = [[0,0], [2,1], [1,1], [0,0]]
  boundary_shift_3_kill  = tf.pad(tf.maximum(boundary_shift_3 - boundary_shift_1, 0.0), padding_3)
  padding_4 = [[0,0], [1,1], [2,1], [0,0]]
  boundary_shift_4_kill  = tf.pad(tf.maximum(boundary_shift_4 - boundary_shift_2, 0.0), padding_4)
  padding_5 = [[0,0], [1,2], [1,2], [0,0]]
  boundary_shift_5_kill  = tf.pad(tf.maximum(boundary_shift_5 - boundary_shift_7, 0.0), padding_5)
  padding_6 = [[0,0], [2,1], [1,2], [0,0]]
  boundary_shift_6_kill  = tf.pad(tf.maximum(boundary_shift_6 - boundary_shift_8, 0.0), padding_6)
  padding_7 = [[0,0], [2,1], [2,1], [0,0]]
  boundary_shift_7_kill  = tf.pad(tf.maximum(boundary_shift_7 - boundary_shift_5, 0.0), padding_7)
  padding_8 = [[0,0], [1,2], [2,1], [0,0]]
  boundary_shift_8_kill  = tf.pad(tf.maximum(boundary_shift_8 - boundary_shift_6, 0.0), padding_8)

  boundary_out = boundary_shift_1_kill + boundary_shift_2_kill + boundary_shift_3_kill + boundary_shift_4_kill + boundary_shift_5_kill + boundary_shift_6_kill + boundary_shift_7_kill + boundary_shift_8_kill - 2
  boundary_out  = tf.maximum(tf.minimum(boundary_out, 1.0), 0.0)
  ...
  boundary_in_a = (boundary_shift_1_kill + boundary_shift_2_kill + boundary_shift_3_kill + boundary_shift_4_kill + boundary_shift_5_kill + boundary_shift_6_kill + boundary_shift_7_kill + boundary_shift_8_kill)
  boundary_in_a  = tf.maximum(tf.minimum(boundary_in_a, 1.0), 0.0)
  boundary_in_b = -((boundary_shift_1_kill + boundary_shift_2_kill + boundary_shift_3_kill + boundary_shift_4_kill + boundary_shift_5_kill + boundary_shift_6_kill + boundary_shift_7_kill + boundary_shift_8_kill) - 4.0)
  boundary_in_b  = tf.maximum(tf.minimum(boundary_in_b, 1.0), 0.0)
  boundary_in = boundary_in_b

  print(boundary_out.get_shape())

  return boundary_out, boundary_in

def calc_drag(boundary, sflow):
  # take of edges of flow
  boundary = boundary[:,1:-1,1:-1,:]
  sflow = sflow[:,1:-1,1:-1,:]

  # compute boundary shifts for all 8 ejacent cells
  boundary_shift_1 = boundary[:,1:  , :  ,0]
  sflow_shift_1    =    sflow[:, :-1, :  ,4]
  boundary_shift_2 = boundary[:, :  ,1:  ,0]
  sflow_shift_2    =    sflow[:, :  , :-1,3]
  boundary_shift_3 = boundary[:, :-1, :  ,0]
  sflow_shift_3    =    sflow[:,1:  , :  ,2]
  boundary_shift_4 = boundary[:, :  , :-1,0]
  sflow_shift_4    =    sflow[:, :  ,1:  ,1]
  boundary_shift_5 = boundary[:,1:  ,1:  ,0]
  sflow_shift_5    =    sflow[:, :-1, :-1,7]
  boundary_shift_6 = boundary[:, :-1,1:  ,0]
  sflow_shift_6    =    sflow[:,1:  , :-1,6]
  boundary_shift_7 = boundary[:, :-1, :-1,0]
  sflow_shift_7    =    sflow[:,1:  ,1:  ,5]
  boundary_shift_8 = boundary[:,1:  , :-1,0]
  sflow_shift_8    =    sflow[:, :-1,1:  ,8]

  boundary_shift_1_kill  = tf.maximum(boundary_shift_1 - boundary_shift_3, 0.0)
  boundary_shift_2_kill  = tf.maximum(boundary_shift_2 - boundary_shift_4, 0.0)
  boundary_shift_3_kill  = tf.maximum(boundary_shift_3 - boundary_shift_1, 0.0)
  boundary_shift_4_kill  = tf.maximum(boundary_shift_4 - boundary_shift_2, 0.0)
  boundary_shift_5_kill  = tf.maximum(boundary_shift_5 - boundary_shift_7, 0.0)
  boundary_shift_6_kill  = tf.maximum(boundary_shift_6 - boundary_shift_8, 0.0)
  boundary_shift_7_kill  = tf.maximum(boundary_shift_7 - boundary_shift_5, 0.0)
  boundary_shift_8_kill  = tf.maximum(boundary_shift_8 - boundary_shift_6, 0.0)

  drag_x = 0.0
  drag_y = 0.0
  drag_y -= tf.reduce_sum(boundary_shift_1_kill * sflow_shift_1)
  drag_x -= tf.reduce_sum(boundary_shift_2_kill * sflow_shift_2)
  drag_y += tf.reduce_sum(boundary_shift_3_kill * sflow_shift_3)
  drag_x += tf.reduce_sum(boundary_shift_4_kill * sflow_shift_4)

  drag_y -= tf.reduce_sum(boundary_shift_5_kill * sflow_shift_5) 
  drag_x -= tf.reduce_sum(boundary_shift_5_kill * sflow_shift_5) 

  drag_y += tf.reduce_sum(boundary_shift_6_kill * sflow_shift_6) 
  drag_x -= tf.reduce_sum(boundary_shift_6_kill * sflow_shift_6) 

  drag_y += tf.reduce_sum(boundary_shift_7_kill * sflow_shift_7) 
  drag_x += tf.reduce_sum(boundary_shift_7_kill * sflow_shift_7) 

  drag_y -= tf.reduce_sum(boundary_shift_8_kill * sflow_shift_8) 
  drag_x += tf.reduce_sum(boundary_shift_8_kill * sflow_shift_8) 

  return drag_x, drag_y, boundary_shift_1_kill * sflow_shift_1
  #return drag_x, drag_y, (boundary_shift_y_plus_kill * sflow_m[:,l+1:-1,:]) - (boundary_shift_y_minus_kill * sflow_m[:,1:-l-1,:])
